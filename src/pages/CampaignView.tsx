...
import { ImageCropper } from '../components/ImageCropper'; // IMPORT RESTORED
...
export const CampaignView = () => {
  ...
  const handlePhotoSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      // Use FileReader for robust mobile support
  ...
  const handleShare = async () => {
    ...
    } catch (e) { try { await navigator.share({ title: campaign?.title, url: window.location.href }); } catch (e2) {} }
  ...
  // Aspect Ratio Safety Logic
  const frameWidth = campaign.frame?.width || 200;
  const frameHeight = campaign.frame?.height || 200;
  // If height is 0 (bad data), default to square (1)
  const safeAspect = frameHeight > 0 ? frameWidth / frameHeight : 1;

  return (
    <div className="min-h-screen aurora-bg py-8 px-4 flex flex-col items-center relative">

      {/* CROPPER COMPONENT (Rendered via Portal) */}
      {showCropper && rawImage && (
        <ImageCropper
          imageSrc={rawImage}
          aspect={safeAspect}
          shape={campaign.frame.shape || 'rect'}
      ...
      <GlassCard className="w-full max-w-lg overflow-hidden relative z-0">
        ...
        <div className="p-8">
          ...
          <div className="space-y-5">
                ...
                <div className="flex flex-col items-center justify-center pt-5 pb-6">
                  {userImage ? (
                    <div className="flex items-center gap-2 text-green-600 font-medium">
                      <ImageIcon size={20} />
                      <span>{userImage ? "Change Photo" : "Upload Selfie"}</span>
                    </div>
                  ) : (
                    <>
            ...
            <div className="flex gap-3 pt-2">
              <GlowButton onClick={handleDownload} disabled={!userImage || (!!campaign.text && !userName) || isGenerating} isLoading={isGenerating && actionType === 'download'} icon={<Download size={18} />} className="flex-1">
                Download
              </GlowButton>
              {showShare && (
                <GlowButton onClick={handleShare} disabled={!userImage || (!!campaign.text && !userName) || isGenerating} variant="secondary" isLoading={isGenerating && actionType === 'share'} icon={<Share2 size={18} />} className="flex-1">
                  Share
                </GlowButton>
              )}
            </div>
